...
use crate::generator::queue::{GenQueue, GenerationJob, JobStatus};
...
impl QueuePanel {
    pub fn new(queue: Arc<GenQueue>) -> Self {
        Self {
            show_panel: true,
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        let completed = jobs.iter().filter(|j| j.status.is_complete()).count();
    ...
    fn show_job_card(&self, ui: &mut Ui, job: &GenerationJob, sender: &mut UiEventSender) {
        egui::Frame::none()
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.vertical(|ui| {
                        ui.horizontal(|ui| {
                            ...
                            ui.label(
                                RichText::new(format!("({})", job.model.name()))
                                    .small()
                                    .color(Color32::GRAY)
                            );
                        });
                        ...
                        let time_str = if let Some(completed) = job.completed_at {
                            let duration = (completed - job.submitted_at).as_secs();
